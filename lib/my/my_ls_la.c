...
void display_right_la(struct dirent *entry, const char *file, struct stat *s)
{
...
int display_la(const char *file, int ac, struct dirent *entry)
{
    struct passwd *pswrd;
    struct group *grp;
    struct stat s;
...
static int count_st_block_la(const char *file)
{
    DIR *dir;
    int nb_block = 0;
    int var;
...
int my_ls_la(const char *file, int ac)
{
    ...
    struct stat s;
    int i = 4;
    int nb_block = count_st_block_la(file);
